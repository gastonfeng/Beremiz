    ...
    def GenerateCFile(self, filepath, location_str, master_number):
        ...
        for (slave_idx, slave_alias, slave) in self.Slaves:
            ...
            if len(device.getTxPdo() + device.getRxPdo()) > 0 or len(slave_variables) > 0 and PdoConfig and PdoAssign:
                ...
                sync1_shift_time = 0
                if dc_enable:
                    sync0_cycle_token = self.Controler.GetChildByIECLocation(
                        (slave_idx,)).BaseParams.getDC_Sync0_Cycle_Time()
                    if sync0_cycle_token != "None":
                        sync0_cycle_time = int(sync0_cycle_token.split("_")[1]) * 1000
                    sync0_shift_token = self.Controler.GetChildByIECLocation(
                        (slave_idx,)).BaseParams.getDC_Sync0_Shift_Time()
                    if sync0_shift_token != "None":
                        sync0_shift_time = int(sync0_shift_token) * 1000
                    sync1_cycle_token = self.Controler.GetChildByIECLocation(
                    ...
                    sync1_shift_token = self.Controler.GetChildByIECLocation(
                    ...
                    dc_config_data = {
                        "slave": slave_idx,
                        "assign_activate": int(
                            self.Controler.GetChildByIECLocation((slave_idx,)).BaseParams.getDC_Assign_Activate()),
                    ...
                    for entry in entries:
                        ...
                        if entry_check_list and check_data in entry_check_list:
                            if (entry_infos["index"] == 0) or (entry_infos["name"] == None):
                                ...
                                entry_infos.update(type_infos)
                                entries_infos.append(
                                    "    {0x%(index).4x, 0x%(subindex).2x, %(bitlen)d}, /* %(name)s */" % entry_infos)
                        ...
                        if entry_declaration is not None and not entry_declaration["mapped"]:
                            ...
                            entry_infos.update(
                                dict(zip(["var_type", "dir", "var_name", "no_decl", "extra_declarations"],
                ...
                if PdoConfig and PdoAssign:
                    ...
                    for (index, subindex), entry_declaration in list(slave_variables.items()):
