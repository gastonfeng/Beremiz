...
class _CommonSlave(object):
    ...
    def GetAllSDOValuesFromSlave(self):
        ...
        error, return_val = self.Controler.RemoteExec(SDO_UPLOAD % (entry_infos, self.Controler.GetSlavePos()),
                                                      return_val=None)
    ...
    def GetSmartViewInfos(self):
        ...
        if device is not None:
            ...
            for vendor_id, vendor in list(self.Controler.CTNParent.CTNParent.ModulesLibrary.Library.items()):
        ...
        if device is not None:
            ...
            for eeprom_element in device.getEeprom().getchildren():
                if eeprom_element.tag == "ByteSize":
                    ...
                    data = "{:0>4x}".format(int(eeprom_size / 1024 * 8 - 1))
    ...
    def ExtractEEPROMStringCategory(self, device):
        ...
        if grouptypeflag is False:
            if self.Controler.CTNParent.CTNParent.ModulesLibrary.Library is not None:
                for _vendor_id, vendor in list(self.Controler.CTNParent.CTNParent.ModulesLibrary.Library.items()):
                    for group_type, group_etc in list(vendor["groups"].items()):
        ...
        if self.Controler.CTNParent.CTNParent.ModulesLibrary.Library is not None:
            for _vendorId, vendor in list(self.Controler.CTNParent.CTNParent.ModulesLibrary.Library.items()):
                for group_type, group_etc in list(vendor["groups"].items()):
                    for device_item in group_etc["devices"]:
                        if device == device_item[1]:
                            data = group_etc["name"]
