...
class SDOPanelClass(wx.Panel):
    def __init__(self, parent, controler):
        ...
        self.SDOMonitorGrid.Bind(wx.grid.EVT_GRID_CELL_LEFT_DCLICK,
...
class SlaveSDOTable(wx.grid.Grid):
    def __init__(self, parent, data, fixed_value):
        ...
        self.Bind(wx.grid.EVT_GRID_CELL_LEFT_DCLICK, self.onGridDoubleClick)
    ...
    def __init__(self, parent, controler):
        ...
        if len(self.rx_pdo_entries):
            for i in range(len(self.rx_pdo_entries)):
                self.PDOcheckBox.append(
                    wx.CheckBox(self, label=str(hex(self.rx_pdo_entries[i]['pdo_index'])), size=(120, 15)))
    ...
    def __init__(self, parent, controler):
        ...
        if len(self.tx_pdo_entries):
            for i in range(len(self.tx_pdo_entries)):
                self.PDOcheckBox.append(
                    wx.CheckBox(self, label=str(hex(self.tx_pdo_entries[i]['pdo_index'])), size=(120, 15)))
...
class SmartViewTreeListCtrl(wx.Panel):
    def __init__(self, parent, Controler):
        ...
        self.Tree = wx.lib.gizmos.TreeListCtrl(self, -1, size=(350, 500),
...
class HexView(wx.Panel):
    ...
    def OnButtonWriteToBinFile(self, event):
        ...
        dialog = wx.FileDialog(self, _("Save as..."), os.getcwd(), "slave0.bin",
                               _("bin files (*.bin)|*.bin|All files|*.*"), wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
...
class SITreeListCtrl(wx.Panel):
    ...
    def __init__(self, parent, controler):
        ...
        self.Tree = wx.lib.gizmos.TreeListCtrl(self, -1, size=wx.Size(750, 350),
        ...
        for slave_idx in range(slave_num):
            ...
            for ec, sub_ecs in [("Port Error Counters 0/1/2/3", [
                ...
                for sub_ec in sub_ecs:
                    ec_sub_idx += 1
                    ec_name = sub_ec
                    tree_node = self.Tree.AppendItem(error_counter[(ec_idx, 0)]["tree_node"], "%s" % sub_ec)
...
class DCConfigPanel(wx.Panel):
    def __init__(self, parent, controler):
        ...
        self.SizerDic = {
            ...
            "DCConfig_inner_main_sizer": wx.FlexGridSizer(cols=1, hgap=50, rows=3, vgap=10),
            "CyclicMode_InnerSizer": wx.FlexGridSizer(cols=1, hgap=5, rows=3, vgap=5),
