...
/********************* Modbus Section ***************************/ 
#define PLC_STATE_MODBUS_READ_PIPE_CREATED     512
#define PLC_STATE_MODBUS_WRITE_PIPE_CREATED    1024
...
/********************* Modbus Section ***************************/ 
#define MODBUS_READ_PIPE             10
#define MODBUS_READ_PIPE_DEVICE      "/dev/rtp10"
#define MODBUS_WRITE_PIPE            11
#define MODBUS_WRITE_PIPE_DEVICE     "/dev/rtp11"
...
/********************* Modbus Section ***************************/ 
RT_PIPE Modbus_Read_pipe;
RT_PIPE Modbus_Write_pipe;
...
/* Code For Slave Based DC by jblee */
/*
extern RTIME calculate_sleeptime(uint64_t wakeup_time);
extern uint64_t calculate_first(void);
extern int slave_dc_used;

uint64_t wakeup_time = 0LL;

void PLC_DC_task_proc(void *arg)
{
    RTIME wakeup_count;
    wakeup_time = calculate_first();
    wakeup_count = calculate_sleeptime(wakeup_time);
    rt_task_sleep_until(wakeup_count);
    ...
    while (!PLC_shutdown) {
        ...
        wakeup_time = wakeup_time + common_ticktime__;
        wakeup_count = calculate_sleeptime(wakeup_time);
        rt_task_sleep_until(wakeup_count);
    }
...
void PLC_cleanup_all(void)
{
    ...
    /********************* Modbus Section ***************************/ 
    if (PLC_state & PLC_STATE_MODBUS_READ_PIPE_CREATED) {
        rt_pipe_delete(&Modbus_Read_pipe);
        PLC_state &= ~PLC_STATE_MODBUS_READ_PIPE_CREATED;
    }

    if (PLC_state & PLC_STATE_MODBUS_WRITE_PIPE_CREATED) {
        rt_pipe_delete(&Modbus_Write_pipe);
        PLC_state &= ~PLC_STATE_MODBUS_WRITE_PIPE_CREATED;
    }
...
int startPLC(int argc,char **argv)
{
    ...
    /********************* Modbus Section ***************************/ 
    /* open Modbus Read Pipe */
    if(rt_pipe_create(&Modbus_Read_pipe, "ModbusReadPipe", MODBUS_READ_PIPE, 0))
        goto error;
    PLC_state |= PLC_STATE_MODBUS_READ_PIPE_CREATED;

    /* open Modbus Read Pipe */
    if(rt_pipe_create(&Modbus_Write_pipe, "ModbusWritePipe", MODBUS_WRITE_PIPE, 0))
        goto error;
    PLC_state |= PLC_STATE_MODBUS_WRITE_PIPE_CREATED;

    if(rt_task_create(&PLC_task, "PLC_task", 0, 50, T_JOINABLE)) goto error;
    //if(rt_task_create(&PLC_task, "PLC_task", 0, 2, T_JOINABLE|T_CPU(3))) goto error;
    PLC_state |= PLC_STATE_TASK_CREATED;
	...
	/*
    if(slave_dc_used == 1) {
        if(rt_task_start(&PLC_task, &PLC_DC_task_proc, NULL)) goto error;
    }
    else {
        if(rt_task_start(&PLC_task, &PLC_task_proc, NULL)) goto error;
    }
...
/******************************** Modbus Section **********************************/
void WriteModbusData(void *buffer, int size){
    rt_pipe_write(&Modbus_Read_pipe, buffer, size, P_NORMAL);
}

int ReadModbusData(void *buffer, int size){
    ssize_t read_size;
    read_size = rt_pipe_read(&Modbus_Write_pipe, buffer, size, TM_NONBLOCK);
    if (read_size > 0) {
        return 1;
    }
    else {
